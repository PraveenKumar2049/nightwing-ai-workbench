...
def main():
    ...
    # Nightwing strip: agent.secret_sources (external secret-manager plumbing)
    # is removed from this build, so this registration is best-effort — same
    # ImportError-tolerant pattern used by tools/environments/docker.py for
    # the sibling agent.proxy_sources plumbing.
    try:
    ...
    # Nightwing strip: agent.proxy_sources (iron-proxy egress firewall) is
    # removed from this build — best-effort registration, see secrets block above.
    try:
