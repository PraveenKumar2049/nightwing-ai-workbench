...
# Nightwing strip: tools.voice_mode removed (voice/TTS cut per air-gap
# requirements). Matches this module's own documented contract above —
# "missing optional audio deps ... surface as an ImportError at call time,
# not at startup" — so callers into this now-always-missing module still
# import it fine and only fail when they actually try to use voice mode.
try:
    ...
    def _voice_mode_unavailable(*args, **kwargs):
        raise ImportError("Voice mode is not available in this build.")

    create_audio_recorder = _voice_mode_unavailable
    is_voice_stop_phrase = _voice_mode_unavailable
    is_whisper_hallucination = _voice_mode_unavailable
    play_audio_file = _voice_mode_unavailable
    transcribe_recording = _voice_mode_unavailable
...
def _speak_text_streaming(text: str, stop_event: Optional[threading.Event] = None) -> bool:
    ...
    # Nightwing strip: tools.tts_tool removed (voice/TTS cut per air-gap
    # requirements) — report no playback so the caller falls back cleanly.
    try:
