...
def _custom_provider_options(
    ...
    *kind* is ``"tts"`` or ``"stt"``. The result keeps the built-in display
    ...
    # Nightwing strip: tools.tts_tool / tools.transcription_tools removed
    # (voice/TTS cut per air-gap requirements) — no built-in providers ship
    # in this build.
    try:
        if kind == "tts":
...
def _split_text_for_speak_stream(text: str, cap: int) -> list:
    ...
    """
    # Nightwing strip: tools.tts_streaming removed (voice/TTS cut per
    # air-gap requirements) — fall back to a plain sentence-boundary regex.
    try:
...
async def speak_stream_ws(ws: "WebSocket") -> None:
    ...
    def _resolve():
        # Nightwing strip: tools.tts_streaming / tools.tts_tool removed
        # (voice/TTS cut per air-gap requirements) — the caller already
        # treats streamer=None as "send a fallback and close".
        try:
    ...
    def _produce():
        # Nightwing strip: unreachable in practice — _resolve() above
        # already proved these modules import fine (or this function is
        # never scheduled), but guard directly anyway.
        try:
