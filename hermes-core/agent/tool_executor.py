...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if function_name == "todo":
            ...
            def _execute(next_args: dict) -> Any:
                # Nightwing strip: tools.read_terminal_tool removed
                # (desktop-GUI tool cut) — unreachable since the tool is no
                # longer registered, but stays import-safe regardless.
                try:
                    from tools.read_terminal_tool import read_terminal_tool as _read_terminal_tool
                except ImportError:
                    return "Error: read_terminal is not available in this build."
