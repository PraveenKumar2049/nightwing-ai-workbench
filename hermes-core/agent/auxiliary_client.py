...
def _build_xai_oauth_aux_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    # Nightwing strip: tools.xai_http removed along with the xai provider
    # plugin — this path requires xAI OAuth to already be configured, which
    # is no longer possible, but stays import-safe regardless.
    try:
        from tools.xai_http import hermes_xai_default_headers
    except ImportError:
        def hermes_xai_default_headers():
            return {}
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        # provider plugin — unreachable via any shipped config.
        try:
            from tools.xai_http import hermes_xai_default_headers
