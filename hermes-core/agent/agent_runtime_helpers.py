...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    if function_name == "todo":
        ...
        def _execute(next_args: dict) -> Any:
            # Nightwing strip: tools.read_terminal_tool removed (desktop-GUI
            # tool cut) — this dispatch branch is unreachable since the tool
            # is no longer registered, but stays import-safe regardless.
            try:
                from tools.read_terminal_tool import read_terminal_tool as _read_terminal_tool
            except ImportError:
                return _finish_agent_tool("Error: read_terminal is not available in this build.", next_args)
            return _finish_agent_tool(
                _read_terminal_tool(
