...
async def _default_url_fetcher(url: str) -> str:
    # Nightwing strip: tools.web_tools removed (no internet access in this
    # build) — @url references can't be fetched, so surface that instead of
    # crashing.
    try:
        from tools.web_tools import web_extract_tool
    except ImportError:
        return "[This build has no internet access — URL references can't be fetched. Download the file locally instead.]"
