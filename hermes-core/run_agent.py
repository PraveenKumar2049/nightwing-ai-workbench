...
# Nightwing strip: tools.browser_tool is removed from this build (browser
# automation cut per air-gap requirements). Lazy + ImportError-tolerant so
# a missing module doesn't break agent init; the one call site below is
# already wrapped in try/except Exception.
try:
    from tools.browser_tool import cleanup_browser
except ImportError:
    def cleanup_browser(task_id):  # type: ignore[misc]
        pass
    ...
    def _apply_client_headers_for_base_url(
        ...
        *,
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            # Nightwing strip: tools.xai_http removed along with the xai
            # provider plugin — this branch is unreachable via any shipped
            # config, but stays import-safe if a base_url is hand-edited.
            try:
